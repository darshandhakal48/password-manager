...
const FooterLink: FC<{ link: string; label: string }> = ({ link, label }) => {
  return (
    <p className="text-lg text-muted-foreground font-semibold hover:text-white cursor-pointer">
      Web
    </p>
  );
};
...
const Footer: FC<React.HTMLAttributes<HTMLDivElement>> = (props) => {
  return (
    <div
      className={cn(
        "asbolute bottom-0 left-0 bg-gray-900 rounded-t-lg py-10",
        props.className,
      )}
    >
      <div className="flex flex-col lg:flex-row items-start justify-center gap-y-10 md:justify-between container gap-x-10">
        <div className="flex-1 space-y-5">
          <Logo />
          <p className="text-lg">
            SAFE is a free and secure way to store your secrets{" "}
          </p>
