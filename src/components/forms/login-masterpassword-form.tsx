...
const MasterPasswordLoginForm: FC<{ session: Session }> = ({ session }) => {
  ...
  async function handleConfirmMasterPassword(event:React.FormEvent<HTMLFormElement>) {
  ...
  return (
    <div className="bg-blue-500 background">
      <div className="backdrop-blur-3xl w-full bg-black/70 h-screen flex items-center justify-center">
        <form onSubmit={handleConfirmMasterPassword} className="w-[90%] lg:w-[30%] bg-white/10 px-10 py-12 rounded-lg my-auto flex-col flex">
          ...
          <Button type="submit" variant="modern" className="text-white mt-10">
            Open My Vault
          </Button>
        </form>
      </div>
