...
const VariantForms: FC<{ variant: ModulesVariantTypes; userInfo: UserDataType }> = ({ variant, userInfo }) => {
  ...
  async function handleConfirmMasterPassword(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
  ...
  return (
    <div>
      ...
      {askForMasterpass ? (
        <form onSubmit={handleConfirmMasterPassword} className="w-fit min-w-[350px] mx-auto text-center bg-primary/15 rounded-lg px-5 py-3 space-y-3 mt-10">
          ...
          />
          <br />
          <Button type="submit" variant="modern" className="text-white">
            Unlock My Item
          </Button>
        </form>
      ) : (
