  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      password: initialValues
        ? decrypt(initialValues.password, userMasterpass) || ""
        : "",
      categoryId: initialValues ? initialValues.categoryId || undefined : "",
      totpKey: initialValues
        ? decrypt(initialValues.totpKey, userMasterpass) || undefined
        : "",
      website: initialValues ? initialValues.website || undefined : "",
      note: initialValues
        ? decrypt(initialValues.note, userMasterpass) || undefined
        : "",
      requireMasterPassword: initialValues
        ? initialValues.requireMasterPassword
        : false,
    });
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="px-5 mt-14 grid gap-x-10 grid-cols-2"
      >
        <div className="border-primary/20 border-b py-5">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-gray-300 font-semibold tracking-wide">
                  Password Label
                </FormLabel>
                <FormControl>
                  <Input
                ...
                <FormControl>
                  <div className="flex items-center gap-x-5">
                    <Input
                      defaultValue={field.value}
                      className={cn(
                        formState === "read" && "bg-transparent w-fit",
                      )}
                      disabled={isPending}
                    ...
                    {formState === "read" && (
                      <div className="gap-x-1 flex items-center">
                        <div className="hover:bg-muted cursor-pointer px-3 py-2 rounded-xl flex items-center justify-center">
                          {otp}
                        </div>
            ...
            render={({ field }) => (
              <FormItem className="flex flex-row bg-primary/15 hover:bg-primary/20 items-start space-x-3 space-y-0 rounded-md border border-primary/20 p-4">
                ...
                <div className="space-y-1 leading-none">
                  <FormLabel>
                    Requires masterpassword everytime to check password
                  </FormLabel>
                  <FormDescription>
                    Check it so that every password requires masterpassword to
                    view/copy password *recommended*
                  </FormDescription>
                </div>
              </FormItem>
            )}
